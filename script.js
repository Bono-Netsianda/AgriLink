...
document.addEventListener('DOMContentLoaded', () => {
        ...
        if (cart.length === 0) {
            ...
            cart.forEach(item => {
                ...
                if (product) {
                    ...
                    cartItemElement.querySelector('.item-price').textContent = `R${product.price.toFixed(2)}${product.unit || ''}`;
                    cartItemElement.querySelector('.item-total').textContent = `R${itemTotal.toFixed(2)}`;
    ...
    function displayProductDetail(productId) {
        ...
        if (!product) {
            ...
            `;
             ...
             productDetailContainer.querySelector('.product-detail-price').textContent = `R${product.price.toFixed(2)}${product.unit || ''}`;
    ...
    function renderProductCardHTML(product) {
        ...
        return `
            <div class="product-card" data-id="${product.id}" data-category="${product.category}" data-name="${product.name}" data-price="${product.price}">
                 ...
                 <h3>${product.name}</h3>
                 <p class="price">R${product.price.toFixed(2)}${product.unit ? ` <span>${product.unit}</span>` : ''}</p>
            </div>
        `;
    }
    ...
    console.log("AgriLink script fully loaded and initialized (v: Card Redesign + Dynamic Cart Title).");
